...
sys2 = ('你是财经短视频选题专家。对选题按爆款潜力评分（0-100），命中以下法则越多越好：'
        ...
        '还要挖掘"财经切入角度"：无论选题是否直接涉及财经，都必须找到财经关联'
        '（股价影响/行业趋势/个人理财启示/消费经济/就业影响等）。'
        '输出纯JSON（不要代码块）：{"topic":"","score":90,"criteria":["法则1"],'
        '"structure":"脚本结构名","financial_angle":"财经切入角度说明"}')
...
print(f"✓ 财经切入：{winner.get('financial_angle', '—')}")

# ───────────────────────────────────────────────────────────────
hr("Agent 3 · 解构洗稿（裁缝拼接法）")
sys3 = ('你是财经短视频口播稿专家，用裁缝拼接法（短视频强钩子+长视频深度内容）写200-300字初稿，'
        '含：强开场钩子（10-15字）、问题展开（100字）、核心洞察（100字）、结尾行动引导（一句话）。'
        '无论选题是否直接涉及财经，必须找到其财经切入点（行业影响/个人理财启示/消费趋势/投资逻辑），'
        '以财经视角为普通投资者/打工人提供有价值的洞察。只输出纯文本口播稿，不要标注结构说明。')
financial_angle = winner.get('financial_angle', '找到该话题的财经影响与投资者视角')
draft = call_llm(sys3,
    f"选题：{winner.get('topic', TOPIC)}。"
    f"财经切入角度：{financial_angle}。"
    f"风格：理性犀利，有信息差，口语化，让普通投资者/从业者有收获。")
print("✓ 口播初稿：")
print(wrap(draft))
